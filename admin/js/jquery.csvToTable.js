	...
	*/
	...
	$.fn.CSVToTable = function(csvFile, options) {
		var defaults = {
			...
			separator: ",",
			startLine: 0,
			jtrt_table_sort: "false",
			jtrt_table_filter: "false"
		};	
		...
		return this.each(function() {
			...
			$.get(csvFile, function(data) {
				var tableHTML = '<div class="jtrt_table_container"><table class="' + options.tableClass + '" data-filtering="' + options.jtrt_table_filter + '" data-sorting="' + options.jtrt_table_sort + '">';
				...
				$.each(lines, function(lineCount, line) {
					if ((lineCount == 0) && (typeof(options.headers) != 'undefined')) {
						...
						$.each(headers, function(headerCount, header) {
							tableHTML += '<th data-breakpoints="" class="' + options.thClass + '">' + header + '</th>';
						});
